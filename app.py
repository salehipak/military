...
# Demmender or supplier
    ...
    if upload_dmd is not None:
        ...
        tokenized_uploaded_dmd_df = pd.DataFrame(columns=['id','title', 'description', 'key_words','tokenized_dmd_title', 'tokenized_dmd_description','tokenized_dmd_key_words'])
    ...
    if upload_prd is not None:
        ...
        tokenized_uploaded_prd_df = pd.DataFrame(columns=['id','title', 'description', 'key_words','tokenized_prd_title', 'tokenized_prd_description','tokenized_prd_key_words'])
...
if button_id:
    ...
    if input_type == 'Supplier':
        ...
        if algo == 'LDA':
          ...
          df = df[['dmd_urlIdentifier', 'dmd_title','dmd_key_words','lda_dmd']].rename(columns={'dmd_title': 'Title', 'dmd_key_words': 'keywords','dmd_urlIdentifier':'ID','lda_dmd':'Label'}).reset_index().iloc[:item_number, :]
          df.index += 1
